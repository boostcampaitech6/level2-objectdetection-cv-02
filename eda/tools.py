...
def load(cfg):
    ...
    train_dict = load_json(cfg.anno_path)
    
    return train_df, train_dict

def filter(df, cfg):pass
...
def hist_ann_per_img(df, cfg, save=True):
...
def box_bbox_area(bbox_df, cfg, save=True):
...
def print_statistics(name, values, write=False, file_path=''):
    ...
    if write:
        with open(file_path, "a") as f: f.write(f"{'min:':>20} {values.min():.2f}\n")
    else: 
        print(f"{'min:':>20} {values.min():.2f}")
    if write:
        with open(file_path, "a") as f: f.write(f"{'max:':>20} {values.max():.2f}\n")
    else: 
        print(f"{'max:':>20} {values.max():.2f}")
    if not isinstance(values, np.float64):
        if write:
            with open(file_path, "a") as f: f.write(f"{'Q1:':>20} {Q1:.2f}\n")
        else:
            print(f"{'Q1:':>20} {Q1:.2f}")
        if write:
            with open(file_path, "a") as f: f.write(f"{'median:':>20} {Q2:.2f}\n")
        else:
            print(f"{'median:':>20} {Q2:.2f}")
        if write:
            with open(file_path, "a") as f: f.write(f"{'Q3:':>20} {Q3:.2f}\n")
        else:
            print(f"{'Q3:':>20} {Q3:.2f}")
    if write:
...
def cal_statisitcs(df, bbox_df, cfg, single=False, write=True):
    remove_output_txt(cfg.output_path)

    if not single:
        print_statistics("각 클래스별 bbox들의 개수", df.class_name.value_counts(), write, cfg.output_path)
        print_statistics("각 클래스별 bbox들의 area의 평균", bbox_df.groupby('class_id').frac_bbox_area.mean()*1024*1024, write, cfg.output_path)
        print_statistics("각 클래스별 bbox들의 aspect_ratio의 평균", bbox_df.groupby('class_id').aspect_ratio.mean(), write, cfg.output_path)

    print_statistics("각 이미지별 bbox들의 개수", df.image_id.value_counts(), write, cfg.output_path)
    print_statistics("각 이미지별 bbox들의 area의 평균", bbox_df.groupby('image_id').frac_bbox_area.mean()*1024*1024, write, cfg.output_path)
    print_statistics("각 이미지별 bbox들의 aspect_ratio의 평균", bbox_df.groupby('image_id').aspect_ratio.mean(), write, cfg.output_path)

    if not single:
        print_statistics("각 이미지별 클래스의 개수", df.groupby('image_id').class_name.nunique(), write, cfg.output_path)