...
for fold, (train_idx, val_idx) in enumerate(cv.split(X, y, groups)):
    ...
    # 필요한 디렉토리 생성
    os.makedirs(f'../../dataset/stratified/fold_{fold}/img/train', exist_ok=True)
    os.makedirs(f'../../dataset/stratified/fold_{fold}/img/val', exist_ok=True)
    ...
    for id in train_ids:
        ...
        shutil.copy(f'../../dataset/{path}', f'../../dataset/stratified/fold_{fold}/img/train/{filename}')

    # 검증 이미지 파일 복사
    for id in val_ids:
        path = id_to_path[id]
        filename = os.path.basename(path)
        shutil.copy(f'../../dataset/{path}', f'../../dataset/stratified/fold_{fold}/img/val/{filename}')

    # 훈련 레이블 복사
    for id in train_ids:
        filename = f'{id:04}.txt'
        src = f'../../dataset/yolo_train/labels/{filename}'
        dst = f'../../dataset/stratified/fold_{fold}/img/train/{filename}'
    ...
    for id in val_ids:
        filename = f'{id:04}.txt'
        src = f'../../dataset/yolo_train/labels/{filename}'
        dst = f'../../dataset/stratified/fold_{fold}/img/val/{filename}'
